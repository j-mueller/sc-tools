...
import Ouroboros.Consensus.Block.Abstract qualified as Ouroboros
...
eraHistory :: EraHistory
eraHistory =
  EraHistory (Ouroboros.mkInterpreter $ Ouroboros.summaryWithExactly list)
 where
...
globals params@NodeParams{npSlotLength} =
  mkShelleyGlobals
