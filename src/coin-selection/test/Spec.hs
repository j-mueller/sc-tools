...
spendTokens :: (MonadMockchain m, MonadError BalanceTxError m, MonadFail m) => C.TxId -> m C.TxId
spendTokens _ = do
...
nativeAssetPaymentTo :: (MonadBlockchain m, MonadMockchain m, MonadError BalanceTxError m, MonadFail m) => C.Quantity -> Wallet -> Wallet -> m C.TxId
...
largeTransactionTest = do
  ...
  runMockchain0IOWith Wallet.initialUTxOs Defaults.nodeParams (failOnError largeDatumTx) >>= \case
    Right (Left{}, view failedTransactions -> [(_, err)]) -> case err of
      ApplyTxFailure (ApplyTxError [_]) -> pure ()
