...
-}
balanceAndSubmit :: (MonadMockchain m, MonadFail m) => Tracer m TxBalancingMessage -> Wallet -> TxBodyContent BuildTx BabbageEra -> m (C.Tx CoinSelection.ERA)
...
on the mockchain, using the default network ID
-}
balanceAndSubmitReturn :: (MonadMockchain m, MonadFail m) => Tracer m TxBalancingMessage -> Wallet -> C.TxOut C.CtxTx C.BabbageEra -> TxBodyContent BuildTx BabbageEra -> m (C.Tx CoinSelection.ERA)
...
-}
payToOperator :: (MonadMockchain m, MonadFail m) => Tracer m TxBalancingMessage -> Wallet -> Operator k -> m (C.Tx C.BabbageEra)
...
-}
payToOperator' :: (MonadMockchain m, MonadFail m) => Tracer m TxBalancingMessage -> Value -> Wallet -> Operator k -> m (C.Tx C.BabbageEra)
