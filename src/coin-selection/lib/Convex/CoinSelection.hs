...
module Convex.CoinSelection (
  ...
  exactScriptExecutionErrorFilter,
) where
...
{- | Variant of @exactScriptExecutionError@ that only matches when the predicate is
true for at least one entry in the script execution error list.
-}
exactScriptExecutionErrorFilter :: forall e era. (AsBalancingError e era) => ((C.ScriptWitnessIndex, Text, [Text]) -> Bool) -> L.Prism' e [(C.ScriptWitnessIndex, Text, [Text])]
exactScriptExecutionErrorFilter p = prism' tobe frombe
 where
  tobe :: [(C.ScriptWitnessIndex, Text, [Text])] -> e
  tobe = review _ScriptExecutionErr
  frombe :: e -> Maybe [(C.ScriptWitnessIndex, Text, [Text])]
