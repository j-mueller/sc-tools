...
-}
mkPair :: (AssetId, Quantity) -> (AssetId, Quantity) -> (Pair, Rational)
mkPair a1 a2 =
  let ((x1, Quantity x2), (y1, Quantity y2)) = if (fst a1 <= fst a2) then (a1, a2) else (a2, a1)
  in (Pair x1 y1, x2 % y2)
...
pairFromOutput :: C.TxOut C.CtxTx C.BabbageEra -> Either PairFromOutputError (Pair, Rational)
...
-}
pairFromValue :: Value -> AssetId -> AssetId -> (Pair, Rational)
