...
-}
addScriptWithdrawal :: (MonadBuildTx era m, C.IsShelleyBasedEra era) => C.NetworkId -> ScriptHash -> C.Quantity -> C.ScriptWitness C.WitCtxStake era -> m ()
addScriptWithdrawal n sh quantity witness = do
...
-}
addWithdrawZeroPlutusV2InTransaction
  :: (MonadBuildTx era m, C.HasScriptLanguageInEra PlutusScriptV2 era, Plutus.ToData redeemer, C.IsShelleyBasedEra era)
  => C.NetworkId -> PlutusScript PlutusScriptV2 -> redeemer -> m ()
addWithdrawZeroPlutusV2InTransaction n script redeemer = do
  let sh = C.hashScript $ C.PlutusScript C.PlutusScriptV2 script
  addScriptWithdrawal n sh 0 $ buildScriptWitness script C.NoScriptDatumForStake redeemer

{-| Add a withdrawal of 0 Lovelace from the rewards account locked by the given Plutus V2 script.
The script is provided as a reference input.
-}
addWithdrawZeroPlutusV2Reference
  :: (MonadBuildTx era m, Plutus.ToData redeemer, C.HasScriptLanguageInEra PlutusScriptV2 era, C.IsShelleyBasedEra era)
  => C.NetworkId -> C.TxIn -> ScriptHash -> redeemer -> m ()
addWithdrawZeroPlutusV2Reference n refTxIn script redeemer = addScriptWithdrawal n script 0 $ buildRefScriptWitness refTxIn C.PlutusScriptV2 C.NoScriptDatumForStake redeemer
