...
class Monad m => MonadBlockchain era m | m -> era where
  ...
  utxoByAddress           :: C.AddressInEra era -> m (C.UTxO era)
  ...
  default utxoByAddress :: (MonadTrans t, m ~ t n, MonadBlockchain era n) => C.AddressInEra era -> m (C.UTxO era)
  utxoByAddress = lift . utxoByAddress
  ...
  default networkId :: (MonadTrans t, m ~ t n, MonadBlockchain era n) => m NetworkId
...
instance (MonadIO m, C.IsShelleyBasedEra era) => MonadBlockchain era (MonadBlockchainCardanoNodeT era m) where
  ...
  utxoByAddress (C.AddressInEra _ addr) =
    runQuery' (C.QueryInEra (C.QueryInShelleyBasedEra C.shelleyBasedEra (C.QueryUTxO (C.QueryUTxOByAddress (Set.singleton $ C.toAddressAny addr)))))
